...
public class TagAliasOperatorHelper {
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        if (jPushMessage.getTags() == null || jPushMessage.getTags().size() ==0) {
            // TODO: 2018/9/27 极光推送 tag为空时 发消息在此设置tag
//            EventBus.getDefault().post(new EventBusBean<String>(0, 10012, ""));
        }
        ...
        if(tagAliasBean == null){
            ExampleUtil.showToast("tags 获取缓存记录失败", context);
            return;
        }
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        ...
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
            ExampleUtil.showToast("CheckTag 获取缓存记录失败", context);
            return;
        }
        if(jPushMessage.getErrorCode() == 0){
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        if (jPushMessage.getAlias() == null || TextUtils.isEmpty( jPushMessage.getAlias())) {
            // TODO: 2018/9/27 极光推送 alias为空时 发消息在此设置alias

//            EventBus.getDefault().post(new EventBusBean<String>(0, 10013, ""));
        }
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
            ExampleUtil.showToast("alias 获取缓存记录失败", context);
            return;
        }
        if(jPushMessage.getErrorCode() == 0){
